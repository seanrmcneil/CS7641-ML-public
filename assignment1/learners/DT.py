...
class DTLearner(learners.BaseLearner):
    ...
    # Adapted from https://github.com/JonathaniTay/CS-7641-assignment-1/blob/master/helpers.py
    # Based on the pruning technique from Mitchell p.70 (Reduced Error Pruning?)
    ...
    def prune(self):
        ...
        for candidate in reversed(candidates):  # Go backwards/leaves up
            ...
            score = self.score(self.value_x, self.value_y) #Score the new tree with the children nodes removed
            if score >= c * best_score: #if the new score is greater than or equal to c * best score
                best_score = score 
