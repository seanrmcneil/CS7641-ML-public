...
class AdultData(DataLoader):
    ...
    # Change ddataset name
    def data_name(self):
        return 'AdultData'

    # The column we are trying to predict which is <50 or >50
    def class_column_name(self):
        return '14'

    def _preprocess_data(self):
        # https://www.ritchieng.com/machinelearning-one-hot-encoding/
        classification = self._data[14].map({' <=50K': 0, ' >50K': 1})
        ...
        self._data = pd.concat([self._data, vec_data], axis=1)
        self._data = pd.concat([self._data, classification], axis=1)
...
class PhishingData(DataLoader):
    # Change path to data here:
    def __init__(self, path='data/phishing.data', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    # The column we are trying to predict phishing or not
    def class_column_name(self):
        return '30'
...
class ContraceptiveMethodChoiceData(DataLoader):
    # Change path to data here:
    def __init__(self, path='data/cmc.data', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    # Change ddataset name
    def data_name(self):
        return 'ContraceptiveMethodChoiceData'

    # The column we are trying to predict no use or not
    def class_column_name(self):
        return '9'

    def _preprocess_data(self):
        classification = self._data[9].map({1:1, 3:0, 2:0})
        # https://www.ritchieng.com/machinelearning-one-hot-encoding/
        to_encode = [1, 2, 6, 7]
        label_encoder = preprocessing.LabelEncoder()
        one_hot = preprocessing.OneHotEncoder()
        ...
        self._data = self._data.drop(to_encode, axis=1)
        self._data = self._data.drop(9, axis=1)
        self._data = pd.concat([self._data, vec_data], axis=1)
        self._data = pd.concat([self._data, classification], axis=1)
